...
function extract(selection: string): TweetContext {
  ...
  return {
    ...
    tweet: article?.querySelector('[data-testid="tweetText"]')?.textContent?.trim() ?? article?.innerText?.trim() ?? selection,
...
chrome.runtime.onMessage.addListener((message: { type?: string; action?: Action }) => {
  ...
  chrome.runtime.sendMessage({ type: "tweet-context", action: message.action, context: extract(selection) }).catch(() => undefined);
});
