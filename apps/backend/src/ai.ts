...
const clean = (value: string) => value.replace(/[\u0000-\u001f\u007f]/g, " ").slice(0, 2000).trim();
...
function evidenceText(results: SearchResult[]) {
  return results.slice(0, 12).map((source, index) => `[${index + 1}] ${source.title}\n${source.snippet}\nURL: ${source.url}`).join("\n\n");
}
...
export async function analyzeContext(selection: string, results: SearchResult[], env: Env = process.env): Promise<AnalysisResult> {
  ...
  return {
    summary: results.length ? `Search found ${results.length} relevant sources. AI summarization is not configured.` : "No reliable web evidence was found.",
    background: results.length ? results[0].snippet : "",
    related: results.slice(0, 5).map((source) => source.url),
  };
}

export async function analyzeClaim(selection: string, results: SearchResult[], env: Env = process.env): Promise<AnalysisResult> {
  const claimType = classify(selection);
  const claims = [clean(selection)];
  if (claimType !== "fact" && claimType !== "mixed") {
    return { summary: "", background: "", related: results.slice(0, 5).map((source) => source.url), claimType, claims, reasoning: "This claim type is not eligible for a factual verdict." };
  }
  ...
  return { summary: "AI claim analysis is not configured.", background: "", related: results.slice(0, 5).map((source) => source.url), claimType, claims, verdict, reasoning: "Evidence was retrieved, but a Gemini API key is required to produce a grounded verdict." };
}
