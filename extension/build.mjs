...
chrome.contextMenus.onClicked.addListener(async (info, tab) => {
  ...
  pendingResult = null;
  try { await chrome.sidePanel.open({ tabId: tab.id }); await chrome.tabs.sendMessage(tab.id, { type: "analyze", action, srcUrl: info.srcUrl }); setTimeout(deliverPendingResult, 250); } catch {}
});
...
await writeFile(
	...
	`
function extract(selection, srcUrl) {
  let article = null;
  if (srcUrl) {
    const imgEl = Array.from(document.querySelectorAll("img")).find((img) => img.src === srcUrl || (srcUrl.includes("pbs.twimg.com/media/") && img.src.includes(srcUrl.split("?")[0])));
    article = imgEl?.closest("article") || null;
  }
  if (!article) {
    const selected = window.getSelection()?.anchorNode;
    article = (selected instanceof Element ? selected.closest("article") : selected?.parentElement?.closest("article")) || document.querySelector("article");
  }
  ...
  const tweetText = article?.querySelector('[data-testid="tweetText"]')?.textContent?.trim() || article?.innerText?.trim().slice(0, 5000) || "";

  const imagesSet = new Set();
  if (srcUrl && srcUrl.startsWith("http")) imagesSet.add(srcUrl);
  if (article) {
    const imgs = article.querySelectorAll('img[src*="pbs.twimg.com/media/"], [data-testid="tweetPhoto"] img');
...
chrome.runtime.onMessage.addListener((message) => {
  ...
  chrome.runtime.sendMessage({ type: "tweet-context", action: message.action, context: extract(window.getSelection()?.toString().trim() || "", message.srcUrl) }).catch(() => undefined);
});
`,
);
...
function renderEmpty() { app.innerHTML = shell("Idle") + '<div class="empty"><div class="icon">✦</div><h2>Make a claim clear</h2><p>Select text or right-click an image in an X post, and choose Know Context or Analyze Claim.</p></div></main>'; }
...
function renderResult(response) {
  const e = response.evidence, a = response.analysis, sources = response.search.results || [], images = response.input.images || [];
  ...
  app.innerHTML = shell("Success") + '<section class="evidence"><div class="strength strength-' + e.strength + '">' + e.strength + ' evidence</div><span>' + e.independentDomains + ' independent domains</span><span>' + e.officialSources + ' official sources</span><span>' + Math.round(e.agreementRatio * 100) + '% agreement</span>' + (e.conflicts ? '<span>' + e.conflicts + ' conflicting sources</span>' : '') + (images.length > 0 ? '<span>📷 ' + images.length + ' image' + (images.length > 1 ? 's' : '') + ' analyzed</span>' : '') + '</section>' + (a.verdict ? '<div class="verdict verdict-' + a.verdict + '">' + escape(a.verdict) + '</div>' : '') + (a.claimType ? '<p class="muted">Claim type: ' + escape(a.claimType) + '</p>' : '') + '<h2>' + (response.action === "claim" ? "Analysis" : "Summary") + '</h2><p>' + escape(a.summary) + '</p>' + (a.reasoning ? '<h2>Reasoning</h2><p>' + escape(a.reasoning) + '</p>' : '') + (a.background ? '<h2>Background</h2><p>' + escape(a.background) + '</p>' : '') + '<h2>Sources (' + sources.length + ')</h2><div class="sources">' + sourceHtml + '</div></main>';
}
renderEmpty();
